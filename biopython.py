class Align:
            ...
            for i in range(1, m + 1):
                for j in range(1, n + 1):
                    if seq1[i - 1] == seq2[j - 1]:
                        ...
                        diagonal_score = score_matrix[i - 1][j - 1] + self.mismatch_penalty
        
                    up_score = score_matrix[i - 1][j] + self.gap_penalty
                    left_score = score_matrix[i][j - 1] + self.gap_penalty
        
                    value = max(diagonal_score, up_score, left_score , 0)
                    score_matrix[i][j] = value
                    if value > largest_score:
                        largest_score = value
                        row = i
                        col = j
    ...
    def print_matrix(self,matrix, seq1, seq2):
        print("         ", end="")
        for char in seq2:
            print(f"{char:4}", end="")
        print()
...
aligned_seq1, aligned_seq2, score, matrix = smith.align(seq1,seq2)
...
smith.print_matrix(matrix, seq1, seq2)